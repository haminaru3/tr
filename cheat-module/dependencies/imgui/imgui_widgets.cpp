...
bool ImGui::ButtonEx(const char* label, const ImVec2 & size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(12, 12, 12, 255), 0, false, 1);
    window->DrawList->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), ImColor(50, 50, 50, 255), 0, false, 1);
...
bool ImGui::ColorBarEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImVec2 p = ImGui::GetWindowPos() + ImVec2(0, 6);
    auto dl = ImGui::GetForegroundDrawList();

	dl->AddRectFilledMultiColor(ImVec2(p.x, p.y - 6), ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 4), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(12, 12, 12));
	dl->AddRectFilledMultiColor(ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 6), ImVec2(p.x + ImGui::GetWindowWidth(), p.y - 4), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(12, 12, 12), ImColor(12, 12, 12));

	dl->AddRectFilledMultiColor(ImVec2(p.x + 1, p.y - 5), ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 3), ImColor(55, 177, 218), ImColor(201, 84, 192), ImColor(201, 84, 192), ImColor(55, 177, 218));
	dl->AddRectFilledMultiColor(ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 5), ImVec2(p.x + ImGui::GetWindowWidth() - 1, p.y - 3), ImColor(201, 84, 192), ImColor(204, 227, 54), ImColor(204, 227, 54), ImColor(201, 84, 192));

	dl->AddRectFilledMultiColor(ImVec2(p.x + 1, p.y - 4), ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 2), black, black, black, black);
	dl->AddRectFilledMultiColor(ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 4), ImVec2(p.x + ImGui::GetWindowWidth() - 1, p.y - 2), black, black, black, black);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	PopFont();
	PushFont(comboarrow_font);

	PushColor(ImGuiCol_Text, ImGuiCol_TextShadow, ImVec4(0.635f, 0.635f, 0.635f, 1.0f));
	RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(141, 3), value_bb.Max, "Z", NULL, NULL, ImVec2(0.0f, 0.0f));
	PopStyleColor();

	PushStyleColor(ImGuiCol_Text, ImVec4(0.635f, 0.635f, 0.635f, 1.0f));
	RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(141, 2), value_bb.Max, "Z", NULL, NULL, ImVec2(0.0f, 0.0f));
	PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2 & size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = CalcItemSize(ImVec2(158, 20), CalcItemWidth(), (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        if (IsKeyPressedMap(ImGuiKey_Backspace) && !is_readonly)
        {
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(25,25,25), false, 0);
        draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(12, 12, 12));
        draw_window->DrawList->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), ImColor(50, 50, 50));
        draw_window->DrawList->AddRect(frame_bb.Min + ImVec2(2, 2), frame_bb.Max - ImVec2(2, 2), ImColor(12, 12, 12));
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImGui::PushClipRect(frame_bb.Min, frame_bb.Max - ImVec2(4, 0), false);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, (g.ActiveId == id) ? ImColor(159, 202, 43, 255) : ImColor(GetColorU32(ImGuiCol_Text)), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            ImGui::PopClipRect();
        }
        ...
        if (render_cursor)
        {
            ...
            ImGui::PushClipRect(frame_bb.Min, frame_bb.Max - ImVec2(4, 0), false);
            draw_window->DrawList->AddLine(cursor_screen_rect.GetBL(), cursor_screen_rect.GetBL() + ImVec2(6, 0), ImColor(159, 202, 43, 255));
            ImGui::PopClipRect();
    ...
    } else
    {
        ...
        {
            ImGui::PushClipRect(frame_bb.Min, frame_bb.Max - ImVec2(4, 0), false);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, (g.ActiveId == id) ? ImColor(159, 202, 43, 255) : ImColor(GetColorU32(ImGuiCol_Text)),
...
bool ImGui::SelectableConfig(const char* label, bool selected, bool state, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiID id = window->GetID(label);
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size(size_arg.x != 0.0f ? size_arg.x : label_size.x, 16.f);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrentLineTextBaseOffset;
    ImRect bb_inner(pos, pos + size);
    ItemSize(size);
    ...
    if (selected) {
        ...
        RenderTextClipped(bb_inner.Min + ImVec2(8, 2), bb.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f));
...
bool ImGui::SelectableConfig(const char* label, bool* p_selected, bool* state, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    if (SelectableConfig(label, *p_selected, *state, flags, size_arg))
    {
        *p_selected = !*p_selected;
        return true;
    }
